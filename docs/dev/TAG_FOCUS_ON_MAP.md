...
O clique no nome da tag continua também abrindo a sidebar de detalhes (`TagSidebarComponent`), comportamento que já existia antes — o foco no mapa foi adicionado, não substituiu nada.
...
| Arquivo | Mudança |
|---|---|
| `map-frontend/src/app/filter-menu/filter-menu.component.html` | `(click)="focusTagOnMap(tag)"` no nome da tag |
| `map-frontend/src/app/filter-menu/filter-menu.component.ts` | novo `@Output() tagFocus` e método `focusTagOnMap(tag)` |
...
`tag.related_locations` é um array de IDs. Reaproveitamos o helper já existente `getLocationById` (usado também por `invokeTagSidebar`) para resolver os IDs em objetos `Location`, e emitimos o evento para o componente pai:
...
focusTagOnMap(tag: Tag) {
  if (!tag.visibility) return;

  const locations = tag.related_locations
...
> **Regra**: se a tag estiver desabilitada (`tag.visibility === false`, ou seja, sem marcadores no mapa), o clique no nome **não** deve focar o mapa — não faria sentido focar locations que não estão sendo exibidas. O template (`filter-menu.component.html`) só liga `(click)="focusTagOnMap(tag)"` na variante *visível* do nome da tag; a variante `tag-name-disabled` não tem esse handler. O guard `if (!tag.visibility) return;` no método é uma segunda camada de proteção, caso o método seja chamado a partir de outro lugar no futuro. O clique continua abrindo a sidebar de detalhes normalmente em ambos os estados — isso não foi alterado.
...
- `tag-sidebar.component.html` já recebe `_selectedLocations` (via `invokeTagSidebar`) mas não renderiza uma lista clicável delas — poderia futuramente ganhar uma lista de locations, cada uma com seu próprio "foco no mapa" individual, reaproveitando `onTagFocus`/`focusMapOnLocations`.
